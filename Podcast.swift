...
struct PodView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                VStack(spacing: 0) {
                    // Filled space with .weeder color
                    Color.weeder
                        .frame(height: geometry.safeAreaInsets.top + 30) // Adjust the height as needed
...
struct WebViewer: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        webView.configuration.allowsInlineMediaPlayback = true // Allow inline media playback
        return webView
    }
